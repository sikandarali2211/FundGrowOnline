  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content" style="background:#072d42;color:#fff;border-radius:15px;">
      ...
      <div class="modal-body">
        ...
        <div class="form-group mb-3">
          <label for="exchangeSource" class="form-label text-warning">Transfer from:</label>
          <select id="exchangeSource" class="form-control" style="background:rgba(255,255,255,0.1);border:1px solid #3bd17a;color:#fff;">
            <option value="balance" selected>Balance Wallet</option>
        ...
        <!-- Hidden commission balance for JavaScript -->
        <div id="commissionBalance" style="display: none;">{{ number_format(auth()->user()->referral_commission_balance ?? 0, 2) }}</div>
...
(function () {
  ...
  const exSourceEl   = document.getElementById('exSourceBalance');
  const exPoolEl     = document.getElementById('exPool');
  const exPrevSourceEl = document.getElementById('exPreviewSource');
  ...
  const exSourceSelect = document.getElementById('exchangeSource');
  const sourceWalletLabel = document.getElementById('sourceWalletLabel');
  const previewSourceLabel = document.getElementById('previewSourceLabel');
  const commissionBalanceEl = document.getElementById('commissionBalance');
  ...
  function currentBalances() {
    ...
    const source = num(exSourceEl.textContent.replace(/[$,]/g,''));
    const pool = num(exPoolEl.textContent.replace(/[$,]/g,''));
    console.log('Current balances - Source:', source, 'Pool:', pool);
    return { source, pool };
  }

  function updateSourceWallet() {
    ...
    if (sourceType === 'balance') {
      ...
      // Get commission amount from hidden element
      const commissionAmount = num(commissionBalanceEl.textContent);
      sourceWalletLabel.textContent = 'Pool Commission';
      previewSourceLabel.textContent = 'Preview Pool Commission';
      exSourceEl.textContent = '$' + fmt(commissionAmount);
      exMaxEl.textContent = fmt(commissionAmount);
    }
    
    // Clear amount input and reset preview
    exAmount.value = '';
    updatePreview();
  }
  ...
  function updatePreview() {
    clearMsg(exMsg);
    const { source, pool } = currentBalances();
    const amt = Math.max(0, num(exAmount.value));
    const max = num(exMaxEl.textContent.replace(/[$,]/g,''));
    const sourceType = exSourceSelect.value;
    
    if (amt > max) {
      const walletName = sourceType === 'balance' ? 'Balance Wallet' : 'Pool Commission';
      showMsg(exMsg, `Amount exceeds ${walletName}. Max available: $${fmt(max)}`, 'warning');
    }
    exPrevSourceEl.textContent = '$' + fmt(Math.max(0, source - amt));
    exPrevPoolEl.textContent = '$' + fmt(pool + amt);
  }

  // Live preview
  exAmount && exAmount.addEventListener('input', updatePreview);

  // Handle dropdown change
  exSourceSelect && exSourceSelect.addEventListener('change', updateSourceWallet);
...
const verifyRes = await fetch('{{ route("security.pin.verify.ajax") }}', {
  ...
  body: JSON.stringify({ security_pin: pin }) // ✅ correct param name
      ...
      // Update source balance based on exchange type
      if (sourceType === 'balance') {
        exSourceEl.textContent = '$' + fmt(data.new_balance_amount);
        exPrevSourceEl.textContent = '$' + fmt(data.new_balance_amount);
        exMaxEl.textContent = fmt(data.new_balance_amount);
      } else {
        exSourceEl.textContent = '$' + fmt(data.new_commission);
        exPrevSourceEl.textContent = '$' + fmt(data.new_commission);
        exMaxEl.textContent = fmt(data.new_commission);
        // Update hidden commission balance
        commissionBalanceEl.textContent = fmt(data.new_commission);
      }
  ...
  $('#exchangemodal').on('shown.bs.modal', function () {
    clearMsg(exMsg);
    const { source, pool } = currentBalances();
    exPrevSourceEl.textContent = '$' + fmt(source);
